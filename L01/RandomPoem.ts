namespace RandomPoem {
    ...
    function getVerse(_subject: string, _verb: string, _object: string): void {
        ...
        let  subjectRandom: number = Math.floor(Math.random() * subject.length);
        let  verbRandom: number = Math.floor(Math.random() * verb.length);
        let  objectRandom: number = Math.floor(Math.random() * object.length);
